...
const coachSchema = z.object({
  ...
  followsCount: z.number().catch(0),
  postsCount: z.number().catch(0),
  isBusinessAccount: z.boolean().catch(false),
  isProfessionalAccount: z.boolean().catch(false),
  profilePicUrl: z.string().optional(),
  niche: z.string().optional(),
  verified: z.boolean().catch(false),
  careerPageUrl: z.string().catch(''),
  contactEmail: z.string().catch(''),
  contactPhone: z.string().catch(''),
  applicationUrl: z.string().catch(''),
  metaAdsLibraryUrl: z.string().catch(''),
  googleAdsLibraryUrl: z.string().catch(''),
  isRunningAds: z.boolean().catch(false),
...
export default function CoachForm({ coach, onSuccess }: CoachFormProps) {
  ...
  const {
    ...
    defaultValues: {
      ...
      externalUrls: coach?.externalUrls || '',
      followersCount: coach?.followersCount || 0,
      followsCount: coach?.followsCount || 0,
      postsCount: coach?.postsCount || 0,
      isBusinessAccount: coach?.isBusinessAccount || false,
      isProfessionalAccount: coach?.isProfessionalAccount || false,
      profilePicUrl: coach?.profilePicUrl || '',
      niche: coach?.niche || '',
      verified: coach?.verified || false,
      careerPageUrl: coach?.careerPageUrl || '',
      contactEmail: coach?.contactEmail || '',
      contactPhone: coach?.contactPhone || '',
      applicationUrl: coach?.applicationUrl || '',
      metaAdsLibraryUrl: coach?.metaAdsLibraryUrl || '',
      googleAdsLibraryUrl: coach?.googleAdsLibraryUrl || '',
      isRunningAds: coach?.isRunningAds || false,
      notes: coach?.notes || '',
      tags: coach?.tags || [],
    },
  });
